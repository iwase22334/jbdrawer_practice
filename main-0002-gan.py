...
def Gl_step(G, x, canvas, num_word):
    ...
    for i in range(num_word):
        ...
        tmp_canvas = canvas + torch.ones(model_parameter["d_image"], model_parameter["d_image"]).to(device)
        x2 = roll_and_fill(tmp_canvas.unsqueeze(0), px0 - px, py0 - py)
...
def D_train(G, D, D_optim, x):
    ...
    for i in range(0, hyper_parameter["num_words"] + 1, frag_num):
        ...
        tmp_canvas = canvas + torch.ones(model_parameter["d_image"], model_parameter["d_image"]).to(device)
        tmp_canvas = tmp_canvas.unsqueeze(0).unsqueeze(0).to(device)
        y_pred = D(tmp_canvas, t_data)
    ...
    loss = loss_real + loss_fake + D_train_boost(G, D, D_optim, x)
    return float(loss)


def D_train_boost(G, D, D_optim, x):
    t_data, f_data = torch.split(x, [1, 1], dim=0)
    t_data, f_data = t_data.to(device), f_data.to(device)
    ...
    for epoch in tqdm(range(n_epoch)):
        ...
        if epoch % 10 == 0:
            for i, (x, _) in enumerate(data_seq_generator()):
                ...
                tmp_canvas = canvas + torch.ones(model_parameter["d_image"], model_parameter["d_image"]).to(device)
                jb.save_image(tmp_canvas, path + "%s-%s-generate.png" % ((epoch + 1), i))
