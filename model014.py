...
class Transformer(nn.Module):
    def __init__(self, d_model, d_vocab, num_heads, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.d_vocab = d_vocab

        self.mh_attention = nn.MultiheadAttention(d_model, num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, x_mask):
        ...
        y = y.masked_fill(mask.unsqueeze(2), 0)
...
class DecisionTransformer(nn.Module):
    ...
    def forward(self, states, actions, timesteps, mask, cstep):
        ...
        # get predictions
        # (B, 2, seq_len, d_model) -> (B, seq_len, vocab)
        q_preds = self.predict_q(x[:, 0])  # predict next action given state

        # (B, seq_len, vocab) -> (B, vocab)
        q_preds = q_preds[torch.arange(q_preds.size(0)), cstep.squeeze(dim=1)]
        return q_preds
