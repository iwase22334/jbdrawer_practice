...
# Train Genertor
class Environment:
    def __init__(self, D, subject_img, random_start=True):
        self.px0 = model_parameter["d_image"] // 2
        self.py0 = model_parameter["d_image"] // 2

        bound = model_parameter["d_image"] // 2 - 1
        if random_start:
            # -31 <= dx <= 30 : 1 <= x <= 62
    ...
    def step(self, action):
        ...
        # Calcurate reward
        st1, st2 = self._observation()
        return (st1.detach(), st2.detach()), torch.tensor(reward).detach()
...
# Init meta variables
memory = []
DQN_policy = model012_2.DQN(model_parameter, device).to(device)
DQN_target = model012_2.DQN(model_parameter, device).to(device)
